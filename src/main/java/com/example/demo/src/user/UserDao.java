...
public class UserDao {
    ...
    public int createUser(PostUserReq postUserReq) {
        String createUserQuery = "insert into User (email, password, name, phoneNum) VALUES (?,?,?,?)"; // 실행될 동적 쿼리문
        Object[] createUserParams = new Object[]{postUserReq.getEmail(), postUserReq.getPassword(), postUserReq.getName(), postUserReq.getPhoneNum()}; // 동적 쿼리의 ?부분에 주입될 값
    ...
    public User getPwd(PostLoginReq postLoginReq) {
        String getPwdQuery = "select userIdx, email, password, name, phoneNum, status, createdAt, updatedAt from User where email = ?"; // 해당 email을 만족하는 User의 정보들을 조회한다.
    ...
    public int checkUserIdx(int userIdx) {
        String checkUserIdxQuery = "select exists(select userIdx from User where userIdx = ?)"; // User Table에 해당 email 값을 갖는 유저 정보가 존재하는가?
